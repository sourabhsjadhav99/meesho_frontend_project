    ...
    <div className="flex justify-between items-center my-6 gap-4">
      {steps.map((step) => (
        <div key={step.number} className="flex-1 relative text-center w-[150px] progressmonile">
          <div
            className={`w-5 h-5 text-[14px] rounded-full mx-auto ${currentStep >= step.number ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 border-2'} flex items-center justify-center`}
          >
            {step.number}
          </div>
          <div className="mt-2 text-gray-700">{step.label}</div>
          {step.number !== steps.length && (
            <div style={{marginTop:"-15px"}} className="absolute top-1/2 transform -translate-y-1/2 left-1/2 w-full h-0.5 bg-gray-300 z-0"></div>
          )}
